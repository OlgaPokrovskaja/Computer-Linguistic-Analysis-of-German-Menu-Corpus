"""
Suche nach den Sprachaspekt Person (Verwendung von Personennamen in  Speisennamen und Speisenbeschreibungen)
...
#Ersetzen, um nur Personenrufnamen zu analysieren: output_file_name = 'results/outputs_language_aspects/personen_ruf.json'
output_file_name = 'results/outputs_language_aspects/personen_name.json'
#Ersetzen, um nur Personenrufnamen zu analysieren: 'datasets/person_ruf.txt'
keywords_file_name = 'datasets/person_name.txt'
...
with open(keywords_file_name, 'r', encoding='utf-8') as keywords_file:
    keywords = [keyword.lower() for keyword in keywords_file.read().splitlines()]
    ...
    with open(output_file_name, 'w', encoding='utf-8') as output_file:
       ...
       #Übertragung der Kategorie, des Bundeslandes, latitude und longitude, URL,  sowie einheitliche Formatierung der Stadt, PLZ und Straße
        for obj in json_data:
            kategorie = obj["kategorie"].lower()

            if "Bundesland" in obj:
                bundesland = obj["Bundesland"].lower()
            else:
                bundesland = "undefined"

            restaurantname = obj["restaurantname"].lower()
            url = obj["url"]
            ...
            if 'adresse_stadt' in obj:
                stadt = obj['adresse_stadt'][0].lower()
            elif 'adresse_stadt_p' in obj:
                plz = obj['adresse_stadt_p'].split(' ')[0]
                stadt = obj['adresse_stadt_p'][0].lower()
            else:
                stadt = "undefined"
            ...
            if 'adresse_straße' in obj:
                straße = obj['adresse_straße'][0].lower()
            elif 'adresse_str_h' in obj:
                straße = obj['adresse_str_h'].lower()
            else:
                straße = "undefined"
            ...
            for dish in obj["speisen"]:
                ...
                doc = (' '.join(speisen)).lower()
                doc2 = (' '.join(beschreiben)).lower()

                splits = [] # Liste für nicht leere Speisennamen und Speisenbeschreibungen
                filtered_splits = [] # Liste für die gefundenen aufwertende Adjektive
                ...
                if filtered_splits:
                    filtered_dishes.append({
                        ...
                        'splits_person_name': filtered_splits,
                        'preis': preis
                    })
